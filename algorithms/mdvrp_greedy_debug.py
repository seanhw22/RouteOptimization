...
class MDVRPGreedy:
        ...
        if len(route) == 0:
            ...
            if position == 0:
                ...
                if self.uses_numpy:
                    ...
                    arrival_time += self.T[vehicle][route[-1]][customer]
            else:
    ...
    def insert_customer(self, vehicle, customer, position):
        ...
        # Update time using travel time matrix
        time_increase = self.calculate_time_increase(vehicle, customer, position)
        self.route_time[vehicle] += time_increase

        print(f'[DEBUG] time_increase = {time_increase}')
    ...
    def solve(self, time_limit=None, max_iterations=None,
        ...
        Returns:
            (solution_dict, status_string) where:
                ...
                status_string: 'feasible', 'timeout', 'max_iterations', or 'infeasible'
        """
        ...
        while self.unallocated:
            ...
            if best is None:
                if verbose:
                    print("\n  [X] TIDAK ada kandidat penyisipan yang feasible!")
                    print("  Proses konstruksi dihentikan.")
                if pbar:
                    pbar.close()
                return self._format_solution(start_time), 'infeasible'
